...
/***
    ...
    {
        QMessageBox::information(this, tr("Open Successfully"), tr("Has successfully opened the serial port") + portName, QMessageBox::Ok);

    }else{
        QMessageBox::critical(this, tr("Open failed"), tr("Failed to open the serial port ") + portName + tr("The serial device does not exist or has been occupied"), QMessageBox::Ok);
        return;
    }
***/
...
/**
  Tamiraa added here for RS232
**/

void FormGPS::readMyCom()
{
    QByteArray temp = myCom->readAll();
    qDebug() << "GPS DATA" << temp.data();
    pn->rawBuffer = temp.data();
}
